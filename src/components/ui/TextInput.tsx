...
export const TextInput = forwardRef<HTMLInputElement, TextInputProps>(
  (
    ...
    const baseInputClasses =
      'w-full px-3 py-1.5 sm:py-2 text-xs sm:text-sm bg-white border border-slate-200 rounded-lg text-slate-900 placeholder:text-slate-400 transition-colors focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none disabled:bg-slate-50 disabled:text-slate-500 disabled:cursor-not-allowed'

    const errorClasses = error
      ? 'border-red-400 focus:border-red-500 focus:ring-red-500'
      : ''

    const leftIconPadding = leftIcon ? 'pl-9' : ''
    const rightIconPadding = rightIcon ? 'pr-9' : ''
